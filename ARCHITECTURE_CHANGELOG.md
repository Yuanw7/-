...
## 十五、Renderer Engine 前端渲染器 (2026-05-14 v1.4)

### 15.1 角色定位

**Role**: 资深建筑测绘工程师

### 15.2 Chain-of-Thought 执行逻辑
...
├─────────────────────────────────────────────────────────────┤
│  步骤2: 中心锚定                                           │
│  └─ 计算几何中心点 (x, y) 适配前端旋转逻辑                 │
├─────────────────────────────────────────────────────────────┤
│  步骤3: 包围盒计算                                          │
│  └─ 输出 2D 投影尺寸 (宽、深) + 离地高度 (z)              │
├─────────────────────────────────────────────────────────────┤
│  步骤4: 交互预设                                            │
│  └─ 材质 → 碰撞阻力映射 (1-10)                            │
└─────────────────────────────────────────────────────────────┘
...
| 函数 | 说明 |
|------|------|
| `state_to_render_json()` | GraphState → 前端渲染 JSON |
| `furniture_to_render_object()` | 单个家具 → RenderObject |
| `calculate_bbox_from_center()` | 中心点 + 尺寸 → 包围盒 |
| `generate_fabric_config()` | 生成 Fabric.js 配置 |

---

## 十六、2D 交互平面图编辑器 (2026-05-14 v1.5)

### 16.1 工具组合

| 层级 | 技术选型 | 理由 |
|------|----------|------|
| 前端画布 | **Fabric.js** | 原生支持拖拽/缩放/旋转，对象可导出 JSON |
| 后端服务 | **FastAPI** | 异步 API + WebSocket 支持 |
| 数学计算 | **Shapely** | OBB 碰撞检测 100% 准确 |
| 实时通信 | **WebSocket** | 用户停止拖拽 → 立即返回审计红线 |
...
│                          ▼                                        │
│  ┌─────────────────────────────────────────────────────────┐    │
│  │ 发送 canvas_state JSON via WebSocket / HTTP POST      │    │
│  └─────────────────────────────────────────────────────────┘    │
└─────────────────────────────────────────────────────────────────┘
                              │
                              ▼
...
│                    FastAPI Backend (后端)                         │
│  ┌─────────────┐  ┌─────────────┐  ┌─────────────┐             │
│  │ /api/audit  │  │ /ws/audit   │  │ /api/generate│            │
│  │ 审计端点    │  │ 实时审计    │  │ AI 布局生成 │             │
│  └─────────────┘  └─────────────┘  └─────────────┘             │
│                          │                                        │
│                          ▼                                        │
│  ┌─────────────────────────────────────────────────────────┐    │
│  │ Shapely: create_rectangle_polygon() 碰撞检测          │    │
│  │ classify_violation() 违规分级 T0/T1/T2               │    │
│  └─────────────────────────────────────────────────────────┘    │
└─────────────────────────────────────────────────────────────────┘
```
...
| 方法 | 路径 | 说明 |
|------|------|------|
| GET | `/` | 返回 floorplan.html |
| POST | `/api/audit` | 审计画布状态 |
| POST | `/api/generate-layout` | AI 生成初始布局 |
| GET | `/api/materials` | 获取材质列表 |
| WS | `/ws/audit` | WebSocket 实时审计 |
| WS | `/ws/sync` | WebSocket 状态同步（多人协作） |

### 16.4 Shapely 碰撞检测

```python
def create_rectangle_polygon(center, size, rotation=0) -> Polygon:
    """根据中心点、尺寸、旋转角度创建矩形多边形"""
    # 支持旋转的 OBB (Oriented Bounding Box) 算法

def check_collision(furniture_list) -> list[dict]:
    """检查所有家具对的碰撞和净距"""
    # 返回: [{source_id, target_id, clearance_mm, intersects}]
```
...
| 2026-05-14 | v1.4 | 新增 Renderer Engine 前端渲染器 |
| 2026-05-14 | v1.5 | 新增 FastAPI + Fabric.js 2D 交互平面图编辑器 |
